...
export function getImage(resources, url: string) {
    const img = resources && resources.getImage(url);
    return img || null;
}

export function drawVectorMarker(ctx: CanvasRenderingContext2D, point, symbol, resources) {
...
export function getVectorMarkerPoints(markerType: string, width: number, height: number) {
