...
const Ajax = {
     ...
     */
    jsonp: function (url: string, callback: AjaxCallBack) {
        // INIT
     ...
     * );
     */
    get: function (url: string, options: AjaxOptions, cb: AjaxCallBack) {
     ...
     * );
     */
    post: function (url: string, options: AjaxOptions, cb: AjaxCallBack) {
        let postData;
        if (!isString(url)) {
            //for compatible
            //options, postData, cb
     ...
     * );
     */
    getArrayBuffer(url: string, options: AjaxOptions, cb: AjaxCallBack) {
        if (isFunction(options)) {
            const t = cb;
            // @ts-ignore
            cb = options;
    ...
    // from mapbox-gl-js
    getImage(img: HTMLImageElement, url: string, options: AjaxOptions) {
        return Ajax.getArrayBuffer(url, options, (err, imgData) => {
            if (err) {
                ...
                (img as any).cacheControl = imgData.cacheControl;
                (img as any).expires = imgData.expires;
     ...
     * @static
     */
    getJSON: function (url: string, options: AjaxOptions, cb: AjaxCallBack) {
        if (isFunction(options)) {
            const t = cb;
            // @ts-ignore
            cb = options;
