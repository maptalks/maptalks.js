...
let TEMP_CANVAS: HTMLCanvasElement;
...
const Canvas = {
    ...
    prepareCanvasFont(ctx: CanvasRenderingContext2D, style) {
     ...
     */
    prepareCanvas(ctx: CanvasRenderingContext2D, style, resources, testing?: boolean) {
        if (!style) {
            return;
        }
    ...
    _createGradient(ctx: CanvasRenderingContext2D, g, extent): CanvasGradient {
        let gradient,
            places = g['places'];
        ...
        if (!g['type'] || g['type'] === 'linear') {
            ...
            console.error('create canvas gradient error ');
    ...
    _setStrokePattern(ctx: CanvasRenderingContext2D, strokePattern, strokeWidth: number, linePatternOffset, resources) {
    ...
    clearRect(ctx: CanvasRenderingContext2D, x1, y1, x2, y2) {
        //@ts-ignore
        ctx.canvas._drawn = false;
        ctx.clearRect(x1, y1, x2, y2);
    },

    fillCanvas(ctx: CanvasRenderingContext2D, fillOpacity, x?: number, y?: number) {
        if (hitTesting) {
            fillOpacity = 1;
        }
        //@ts-ignore
    ...
    normalizeColorToRGBA(fill, opacity = 1): string {
        return `rgba(${fill[0] * 255},${fill[1] * 255},${fill[2] * 255},${(fill.length === 4 ? fill[3] : 1) * opacity})`;
    },

    image(ctx: CanvasRenderingContext2D, img, x: number, y: number, width?: number, height?: number) {
        //@ts-ignore
        ctx.canvas._drawn = true;
        try {
    ...
    text(ctx: CanvasRenderingContext2D, text, pt, style, textDesc) {
        Canvas._textOnMultiRow(ctx, textDesc['rows'], style, pt, textDesc['size'], textDesc['rawSize']);
    },

    _textOnMultiRow(ctx: CanvasRenderingContext2D, texts, style, point, splitTextSize, textSize) {
    ...
    _textOnLine(ctx: CanvasRenderingContext2D, text, pt, textHaloRadius, textHaloFill, textHaloAlpha) {
        if (hitTesting) {
            textHaloAlpha = 1;
        }
    ...
    fillText(ctx: CanvasRenderingContext2D, text, pt, rgba?: any) {
        //@ts-ignore
        ctx.canvas._drawn = true;
        if (rgba) {
            ctx.fillStyle = rgba;
    ...
    _stroke(ctx: CanvasRenderingContext2D, strokeOpacity, x?: number, y?: number) {
        if (hitTesting) {
            strokeOpacity = 1;
        }
        //@ts-ignore
    ...
    _path(ctx: CanvasRenderingContext2D, points, lineDashArray?, lineOpacity?, ignoreStrokePattern?: any) {
    ...
    path(ctx: CanvasRenderingContext2D, points, lineOpacity, fillOpacity?: number, lineDashArray?: any) {
        if (!isArrayHasData(points)) {
            return;
        }
        ctx.beginPath();
    ...
    _multiClip(ctx: CanvasRenderingContext2D, points) {
    ...
    polygon(ctx: CanvasRenderingContext2D, points, lineOpacity, fillOpacity, lineDashArray?: any, smoothness?: boolean) {
        // if MultiPolygon clip
        if ((ctx as any).isMultiClip) {
            Canvas._multiClip(ctx, points);
            return;
        }
        //polygon clip
        if ((ctx as any).isClip) {
    ...
    _ring(ctx: CanvasRenderingContext2D, ring, lineDashArray, lineOpacity, ignorePattern?: boolean) {
    ...
    paintSmoothLine(ctx: CanvasRenderingContext2D, points, lineOpacity, smoothValue, close, tailIdx?: any, tailRatio?: any) {
        if (!points) {
            return;
        }
        ...
        for (let i = 0; i < l; i++) {
            ...
            if (i === l - 1 && tailRatio >= 0 && tailRatio < 1) {
                ...
                (lastPoint as any).prevCtrlPoint = new Point(ctrlPoints[2], ctrlPoints[3]);
    ...
    //各种图形的绘制方法
    ellipse(ctx: CanvasRenderingContext2D, pt, width, heightTop, heightBottom, lineOpacity: number, fillOpacity: number) {
    ...
    rectangle(ctx: CanvasRenderingContext2D, pt, size, lineOpacity: number, fillOpacity: number) {
        // pt = pt._round();
        const { x, y } = pt;
        ctx.beginPath();
    ...
    sector(ctx: CanvasRenderingContext2D, pt, size, angles, lineOpacity: number, fillOpacity: number) {
        const rad = RADIAN;
    ...
    drawCross(ctx: CanvasRenderingContext2D, x, y, lineWidth: number, color) {
        (ctx.canvas as any)._drawn = true;
    ...
    copy(canvas: HTMLCanvasElement, c): HTMLCanvasElement {
    ...
    // pixel render
    pixelRect(ctx: CanvasRenderingContext2D, point, lineOpacity: number, fillOpacity: number) {
...
function drawDashLine(ctx: CanvasRenderingContext2D, startPoint, endPoint, dashArray) {
...
function copyProperties(ctx: CanvasRenderingContext2D, savedCtx: CanvasRenderingContext2D) {
