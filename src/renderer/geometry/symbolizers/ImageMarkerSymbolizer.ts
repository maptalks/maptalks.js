...
export default class ImageMarkerSymbolizer extends PointSymbolizer {
    ...
    symbolize(ctx: CanvasRenderingContext2D, resources: ResourceCache): void {
        ...
        if (!img) {
            if (typeof console !== "undefined") {
                console.warn(
                    "no img found for " +
                        (this.style["markerFile"] || this._url[0])
                );
            }
            return;
        }
        ...
        if (
            ...
            isNumber(style["markerOpacity"]) &&
            style["markerOpacity"] < 1
        ) {
    ...
    getFixedExtent(resources: ResourceCache): PointExtent {
        ...
        return getImageMarkerFixedExtent(
            this._fixedExtent,
            this.style,
            resources
        );
    ...
    translate(): any {
        ...
        return {
            ...
            markerHorizontalAlignment: getValueOrDefault(
                s["markerHorizontalAlignment"],
                "middle"
            ), //left | middle | right
            markerVerticalAlignment: getValueOrDefault(
