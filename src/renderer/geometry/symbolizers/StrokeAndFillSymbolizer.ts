...
export default class StrokeAndFillSymbolizer extends CanvasSymbolizer {
    ...
    static test(symbol: any, geometry: Geometry): boolean {
        ...
        if (geometry && (geometry as any).isPoint) {
            return false;
        }
    ...
    symbolize(ctx: CanvasRenderingContext2D, resources: ResourceCache) {
        ...
        const isGradient = checkGradient(style["lineColor"]),
            isPath =
                this.geometry.getJSONType() === "Polygon" ||
                this.geometry.type === "LineString";
        if (isGradient && (style["lineColor"]["places"] || !isPath)) {
            style["lineGradientExtent"] = this.geometry
                .getContainerExtent()
                ._expand(style["lineWidth"]);
        }
        if (checkGradient(style["polygonFill"])) {
            style["polygonGradientExtent"] = this.geometry.getContainerExtent();
        }
        // const lineWidth = style['lineWidth'] || 1;
        const geometryEventTolerance =
            this.geometry.getLayer().options["geometryEventTolerance"] || 0;
        const tolerance =
            this.geometry._hitTestTolerance() + geometryEventTolerance;
    ...
    translate(): any {
        ...
        const result = {
            ...
            polygonPatternFile: getValueOrDefault(
                s["polygonPatternFile"],
                null
            ),
    ...
    _createGradient(
        ...
        if (!p1 || !p2) {
            console.error(
                "unable create canvas LinearGradient,error data:",
                points
            );
            return;
        }
