...
} from "../../../core/util/marker";
import {
    drawVectorMarker,
    translateMarkerLineAndFill,
} from "../../../core/util/draw";
...
export default class VectorMarkerSymbolizer extends PointSymbolizer {
    ...
    constructor(symbol: any, geometry: Geometry, painter: Painter) {
        ...
        this.strokeAndFill = this._defineStyle(
            translateMarkerLineAndFill(this.style)
        );
    ...
    symbolize(ctx: CanvasRenderingContext2D, resources: ResourceCache) {
        ...
        if (
            !this.painter.isHitTesting() &&
            (style["markerWidth"] === 0 ||
                style["markerHeight"] === 0 ||
    ...
    _drawMarkers(
        ...
        for (let i = cookedPoints.length - 1; i >= 0; i--) {
            ...
            const origin = this.getRotation()
                ? this._rotate(ctx, point, this._getRotationAt(i))
                : null;
            if (origin) {
                const pixel = point.sub(origin);
                point = origin;
                const rad = this._getRotationAt(i);
                extent = getMarkerRotationExtent(
                    TEMP_EXTENT,
                    rad,
                    width,
                    height,
    ...
    _drawMarkersWithCache(
        ...
        const anchor = getVectorMarkerAnchor(
            this.style,
            image.width,
            image.height
        );
        ...
        if (!this._stamp) {
            this._stamp = hashCode(
                [
                    ...
                    isGradient(this.style["markerFill"])
                        ? getGradientStamp(this.style["markerFill"])
                        : this.style["markerFill"],
                    this.style["markerFillOpacity"],
                    this.style["markerFillPatternFile"],
                    isGradient(this.style["markerLineColor"])
                        ? getGradientStamp(this.style["markerLineColor"])
                        : this.style["markerLineColor"],
                    this.style["markerLineWidth"],
                    this.style["markerLineOpacity"],
                    this.style["markerLineDasharray"]
    ...
    getFixedExtent(): PointExtent {
        ...
        return getVectorMarkerFixedExtent(
            this._fixedExtent,
            this.style,
            isDynamic ? [128, 128 * (w === 0 ? 1 : h / w)] : null
        );
    }
    ...
    translate(): any {
        ...
        const result = {
            ...
            markerFillPatternFile: getValueOrDefault(
                s["markerFillPatternFile"],
                null
            ),
            markerLineColor: getValueOrDefault(s["markerLineColor"], "#000"), //black
            markerLineWidth: getValueOrDefault(
                s["markerLineWidth"],
                DEFAULT_MARKER_SYMBOLS.markerLineWidth
            ),
            markerLineOpacity: getValueOrDefault(s["markerLineOpacity"], 1),
            markerLineDasharray: getValueOrDefault(
                s["markerLineDasharray"],
                []
            ),
            markerLinePatternFile: getValueOrDefault(
                s["markerLinePatternFile"],
                null
            ),
            ...
            markerWidth: getValueOrDefault(
                s["markerWidth"],
                DEFAULT_MARKER_SYMBOLS.markerWidth
            ),
            markerHeight: getValueOrDefault(
                s["markerHeight"],
                DEFAULT_MARKER_SYMBOLS.markerHeight
            ),
        ...
        result["markerHorizontalAlignment"] = getValueOrDefault(
            s["markerHorizontalAlignment"],
            ha
        ); //left | middle | right
        result["markerVerticalAlignment"] = getValueOrDefault(
            s["markerVerticalAlignment"],
            va
        ); // top | middle | bottom
