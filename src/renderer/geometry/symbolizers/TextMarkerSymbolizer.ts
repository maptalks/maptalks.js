...
export default class TextMarkerSymbolizer extends PointSymbolizer {
    ...
    constructor(symbol: any, geometry: Geometry, painter: Painter) {
        ...
        this.strokeAndFill = this._defineStyle(
            this.translateLineAndFill(this.style)
        );
    }
    ...
    symbolize(ctx: CanvasRenderingContext2D, resources: ResourceCache) {
        ...
        if (
            ...
            (this.style["textSize"] === 0 ||
                (!this.style["textOpacity"] &&
                    (!this.style["textHaloRadius"] ||
                        !this.style["textHaloOpacity"])) ||
        ...
        const textContent = replaceVariable(
            this.style["textName"],
        ...
        const textDesc = (this._textDesc =
            this._textDesc || describeText(textContent, this.style));
        ...
        for (let i = 0, len = cookedPoints.length; i < len; i++) {
            ...
            const origin = this.getRotation()
                ? this._rotate(ctx, p, this._getRotationAt(i))
                : null;
            ...
            if (origin) {
                ...
                const { width, height } = textDesc.size || {
                    width: 0,
                    height: 0,
                };
                const alignPoint = getAlignPoint(
                    textDesc.size,
                    style["textHorizontalAlignment"],
                    style["textVerticalAlignment"]
                );
    ...
    getFixedExtent(): PointExtent {
        ...
        return getTextMarkerFixedExtent(
            this._fixedExtent,
            this.style,
            textDesc
        );
    ...
    translate(): any {
        ...
        const result = {
            ...
            textHorizontalAlignment: getValueOrDefault(
                s["textHorizontalAlignment"],
                "middle"
            ), //left | middle | right | auto
            textVerticalAlignment: getValueOrDefault(
        ...
        if (
            result["textMaxWidth"] > 0 &&
            (!result["textWrapWidth"] ||
