    ...
    _getRenderPoints(placement) {
        ...
        if (placement === 'vertex') {
            ...
            for (let i = 0, len = shell.length; i < len; i++) {
                points.push(map._coordToPointAtRes(shell[i], res));
            }
            return [points, null];
        } else {
            const c = map._coordToPointAtRes(this.getCenter(), res);
            return [
                [c], null
            ];
        }
...
const PolyRenderer = {
    _getRenderPoints(placement) {
        ...
        if (placement === 'point') {
            points = this._getPath2DPoints(this._getPrjCoordinates(), false, res);
            ...
            points = [];
            rotations = [];
            const vertice = this._getPath2DPoints(this._getPrjCoordinates(), false, res),
            ...
            points = coords.length ? [map._prjToPointAtRes(coords[0], res)] : [];
            rotations = coords.length ? [[map._prjToPointAtRes(coords[0], res), map._prjToPointAtRes(coords[1], res)]] : [];
        } else if (placement === 'vertex-last') {
            const coords = this._getPrjCoordinates();
            const l = coords.length;
            points = l ? [map._prjToPointAtRes(coords[l - 1], res)] : [];
            const current = l - 1,
                previous = l > 1 ? l - 2 : l - 1;
            rotations = l ? [[map._prjToPointAtRes(coords[previous], res), map._prjToPointAtRes(coords[current], res)]] : [];
        } else {
            const pcenter = this._getProjection().project(this.getCenter());
            points = [map._prjToPointAtRes(pcenter, res)];
        }
