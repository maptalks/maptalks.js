...
class InfoWindow extends UIComponent {
    ...
    getOffset() {
        ...
        if (isPointGeometry(owner) || isMultiPointGeometry(owner)) {
            let painter, markerSize;
            if (isPointGeometry(owner)) {
                //@ts-ignore
    ...
    _onAutoOpen(e) {
        const owner = this.getOwner();
        setTimeout(() => {
            if (isPointGeometry(owner) || owner instanceof UIComponent) {
                //@ts-ignore
                this.show(owner.getCoordinates());
            } else if (isMultiPointGeometry(owner)) {
                //@ts-ignore
                this.show(owner.findClosest(e.coordinate));
            } else if (isLineStringGeometry(owner) || isMultiLineStringGeometry(owner)) {
