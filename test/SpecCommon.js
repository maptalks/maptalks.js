...
export function genAllTypeGeometries() {
    ...
    return [
        new Marker(center),
        new Label('test label', center),
        new TextBox('test textbox', center),
        new Circle(center, r),
        new Ellipse(center, w, h),
        new Rectangle(center, w, h),
        new Sector(center, r, 90, 180),
        new LineString([{
            x: 121.111,
            y: 30.111
        }, {
 ...
 */
export function commonSetupMap(center, baseLayer) {
    ...
    var map = new Map(container, option);
    if (baseLayer === undefined) {
        var tile = new TileLayer('tile', {
 ...
 * 共同的地图销毁方法
 */
export function commonTearDownMap() {
}

export var GeoSymbolTester = {
    markerSymbols: [{
    ...
    testGeoSymbols: function (geometry, map, done) {
        ...
        function getLayer(id) {
            counter++;
            var layer = map.getLayer(id) || new VectorLayer(id, {
                'debug': true
            }).addTo(map);
        ...
        // geometry.remove();
        // var layer = map.getLayer('symboltest_layer_svg') || new VectorLayer('symboltest_layer_svg').addTo(map);
        ...
        // }
        // geometry.remove();
        // layer = map.getLayer('symboltest_layer_canvas') || new VectorLayer('symboltest_layer_canvas',{'render':'canvas'}).addTo(map);
        // layer.config('drawImmediate' , true);
        // // map.addLayer(layer);
        // layer.addGeometry(geometry);
        // for (i = this.markerSymbols.length - 1; i >= 0; i--) {
 ...
 * @type {Object}
 */
export var GeoEventsTester = function () {};
...
GeoEventsTester.prototype = {
    ...
    testCanvasEvents: function (vector, map, testPoint) {
        var layer = new VectorLayer('event_test_layer_canvas', {
            'render': 'canvas'
        });
