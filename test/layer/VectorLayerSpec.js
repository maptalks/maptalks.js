describe('VectorLayer.Spec', function () {
        ...
        it('repaint when map resized', function (done) {
            ...
            layer.once('layerload', function () {
                ...
                layer.once('layerload', function () {
                    ...
                    maptalks.Browser.retina = false;
                    maptalks.Browser.monitorDPRChange = true;
                    done();
                });
                maptalks.Browser.monitorDPRChange = false;
    ...
    describe('layer mask', function () {
        ...
        function getCircle(radius = 20) {
            return new maptalks.Circle(map.getCenter(), radius, {
                symbol: {
        ...
        function getLayerColor(x, y) {
            const render = map.getRenderer();
            const canvas = render.canvas;
            const ctx = canvas.getContext('2d');
            const { width, height } = canvas;
            x = Math.round(width / 2 + x), y = Math.round(height / 2 + y);
        ...
        it('inside mask', function (done) {
            map.config({ centerCross: false });
            const mask = getMask();
            layer = new maptalks.VectorLayer('v', { 'drawImmediate': true }).addTo(map);
            layer.addGeometry(getCircle());
            layer.setMask(mask);
            setTimeout(() => {
                expect(getLayerColor(0, 0)).to.be.eql(color);
                expect(getLayerColor(10, 0)).not.to.be.eql(color);
                done();
            }, 20);
        });

        it('outside mask', function (done) {
            map.config({ centerCross: false });
            ...
            layer.setMask(mask);
            setTimeout(() => {
                expect(getLayerColor(0, 0)).not.to.be.eql(color);
                expect(getLayerColor(10, 0)).to.be.eql(color);
                done();
            }, 20);
        });
    });
