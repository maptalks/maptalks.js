...
export default class TrafficScene {
    ...
    _update() {
        ...
        while(i--){ this._removeCar( this.toRemove[i]); }
        this.clearTmpRemove();
        const carsMap = this.cars.all();
        for (id in carsMap) {
            this._addCar(carsMap[id], id);
        }
    ...
    _addCar(car) {
        ...
        if(!this.carsList[id]) {
            ...
            const map = this.map;
            const coord = map.pointAtResToCoordinate(new maptalks.Point(p.x / this.gridSize, p.y / this.gridSize), map.getGLRes());
    ...
    get instantSpeed() {
        const speeds = map(this.cars.all(), car => {
            return car.speed;
        });
        if (speeds.length === 0) {
            return 0;
        }
        return (reduce(speeds, function (a, b) { return a + b; })) / speeds.length;
    }
    ...
    intersection(segment, compareLineIndex, segmentIndex, lines, intersectPoints, lineLengthMap) {
        ...
        for (let i = compareLineIndex + 1; i < lines.length; i++) {
            ...
            const point = {
                inLines,
                coordinate: this.map.coordinateToPointAtRes({ x: result.coord[0], y: result.coord[1] }, this.map.getGLRes())
            };
    ...
    generateTraffic(lines) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = 0; j < compareLine.length - 1; j++) {
                ...
                this.intersection(segment, i, intersectCount + j, lines, intersectPoints, lineLengthMap);
            }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = 0; j < line.length; j++) {
                const coordinate = map.coordinateToPointAtRes({ x: line[j][0], y: line[j][1] }, map.getGLRes());
                const inLines = {};
                if (j === 0) {
                    inLines[i] = { pre : null };
