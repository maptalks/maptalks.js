...
const shaderConfig = {
    ...
    vert,
    frag,
    wgslVert: getWGSLSource("layer_video_vert"),
    wgslFrag: getWGSLSource("layer_video_frag"),
    uniforms: [
        {
...
class VideoLayerRenderer extends CanvasCompatible(
    maptalks.renderer.LayerAbstractRenderer,
) {
    constructor(layer) {
        super(layer);
        this._scenes = {};
    ...
    _updateShader() {
        ...
        shaderConfig.extraCommandProps.cull.enable = this.layer.options[
            "doubleSide"
        ]
            ? false
            : true;
    ...
    _createScene(videoSurface) {
        ...
        const { worldCenter, points } =
            this._transformCoordToWorld(coordinates);
    ...
    _transformCoordToWorld(coordinates) {
        ...
        if (map) {
            coordinates.forEach((coordinate) => {
                ...
                const altitude = map.altitudeToPoint(
                    coordinate[2] || 0,
                    map.getGLRes(),
                );
