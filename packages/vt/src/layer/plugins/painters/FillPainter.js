...
class FillPainter extends BasicPainter {
    ...
    supportRenderMode(mode) {
        if (
            this.sceneConfig.antialias ||
            this.sceneConfig.antialias === undefined
        ) {
    ...
    createMesh(geo, transform, params) {
        ...
        const tileCoord = map.pointAtResToCoord(
            COORD0.set(tilePoint[0], tilePoint[1]),
            tileRes,
        );
        const uniforms = {
        ...
        if (isFunctionDefinition(symbolDef["polygonPatternFileOrigin"])) {
            this._preparePatternOrigin(
                symbolDef,
                geo,
                isVectorTile ? [0, 0] : tilePoint,
                tileRes,
            );
        }
        if (
            isFunctionDefinition(symbolDef["polygonPatternFileWidth"]) ||
            isFunctionDefinition(symbolDef["polygonPatternFileWidth"])
        ) {
            this._preparePatternWidth(
                symbolDef,
                geo,
                isVectorTile ? tileRatio : 1,
                tileCoord,
                tileRes,
            );
        }
        if (
            symbolDef["uvOffsetInMeter"] &&
            isFunctionDefinition(symbolDef["uvOffset"])
        ) {
        ...
        setUniformFromSymbol(
            ...
            symbol,
            "polygonOpacity",
            DEFAULT_UNIFORMS["polygonOpacity"],
        );
        setUniformFromSymbol(
            uniforms,
            "uvScale",
        ...
        if (iconAtlas && geometry.data.aTexInfo) {
            ...
            Object.defineProperty(uniforms, "uvOrigin", {
                ...
                get: () => {
                    ...
                    if (geometry.data.aPatternOrigin) {
                        tilePointUniform[0] =
                            (tilePoint[0] * tileScale) % maxIconSize;
                        tilePointUniform[1] =
                            (tilePoint[1] * tileScale) % maxIconSize;
                    ...
                    return vec2.set(
                        tilePointUniform,
                        tilePoint[0] - COORD1.x,
                        tilePoint[1] - COORD1.y,
                    );
                },
            });
            ...
            Object.defineProperty(uniforms, "patternWidth", {
                ...
                get: () => {
                    ...
                    const [width, height] = this._computePatternWidth(
                        ARR_0,
                        texWidth,
                        texHeight,
                        isVectorTile ? tileRatio : 1,
            ...
            Object.defineProperty(uniforms, "patternOffset", {
                ...
                get: () => {
                    ...
                    const offsetX = meterToPoint(
                        map,
                        offset[0],
                        tileCoord,
                        tileRes,
                    );
                    const offsetY = meterToPoint(
                        map,
                        offset[1],
                        tileCoord,
                        tileRes,
            ...
            Object.defineProperty(uniforms, "tileScale", {
                ...
                get: function () {
                    ...
                    return (
                        geometry.properties.tileResolution / map.getResolution()
                    );
                },
            });
            uniforms.polygonPatternFile = createAtlasTexture(
                this.regl,
                iconAtlas,
                false,
                false,
    ...
    _preparePatternWidth(symbolDef, geo, tileRatio, tileCoord, tileRes) {
        ...
        for (let i = 0, l = aPickingId.length; i < l; i++) {
            ...
            width = widthFn
                ? widthFn(null, feature.feature.properties)
                : widthSymbol;
            height = heightFn
                ? heightFn(null, feature.feature.properties)
                : heightSymbol;
            current = aPickingId[i];
            if (width || height) {
                let origin = tileCoord;
                if (aPatternOrigin) {
                    const patternOrigin = originFn(
                        null,
                ...
                const [scaleX, scaleY] = this._computePatternWidth(
                    ARR_0,
                    width,
                    height,
                    tileRatio,
    ...
    _preparePatternOffset(symbolDef, geo, tileCoord, tileRes) {
        ...
        const map = this.getMap();
        let isMeterFn =
            isFunctionDefinition(symbolDef["uvOffsetInMeter"]) &&
            piecewiseConstant(symbolDef["uvOffsetInMeter"]);
    ...
    isEnableTileStencil(context) {
        const isVectorTile = this.layer.getJSONType() === "VectorTileLayer";
        ...
        return (
            isEnableStencil &&
            (isVectorTile || (isTileLayer && this.isOnly2D()))
        );
    }
    ...
    _getExtraCommandProps() {
        ...
        const wgslVert = getWGSLSource("vt_fill_vert");
        const wgslFrag = getWGSLSource("vt_fill_frag");
    ...
    getUniformValues(map, context) {
        ...
        const uniforms = {
            ...
            maskViewport:
                isRenderingTerrainSkin && context && context.maskViewport,
    ...
    _computePatternWidth(
        ...
        if (texHeight) {
            const pointHeight = meterToPoint(
                map,
                texHeight,
                tileCoord,
                tileRes,
