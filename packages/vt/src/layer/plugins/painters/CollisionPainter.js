...
export default class CollisionPainter extends BasicPainter {
    createGeometry(...args) {
        ...
        const glData = args[0];
        geometry.properties.collideIds =
            glData.featureIds && glData.featureIds.length && glData.isIdUnique
                ? glData.featureIds
                : glData.data.aPickingId;
    ...
    supportRenderMode(mode) {
        const renderToPointRenderTarget =
            this.sceneConfig.renderToPointRenderTarget;
        if (
            renderToPointRenderTarget ||
            renderToPointRenderTarget === undefined
        ) {
            return mode === "point";
        } else {
    ...
    addMesh(meshes, progress, context) {
        if (meshes && !this.isEnableCollision()) {
            ...
            for (let i = 0; i < meshesToCheck.length; i++) {
                ...
                const { elements, visElemts } =
                    meshesToCheck[i].geometry.properties;
                if (
                    visElemts &&
                    visElemts.count !== undefined &&
    ...
    startMeshCollision(mesh) {
        ...
        const isForeground = renderer.isForeground(
            mesh instanceof CollisionGroup ? mesh.meshes[0] : mesh,
        );
    ...
    endMeshCollision(meshKey) {
        ...
        if (this._canProceed && meshContext && this._meshCollisionStale) {
            ...
            meshContext.anchor0 = map.containerPointToCoord(
                this._containerAnchor0,
                this._anchorCoord0,
            );
            meshContext.anchor1 = map.containerPointToCoord(
                this._containerAnchor1,
                this._anchorCoord1,
            );
    ...
    _isCachedCollisionStale(meshKey) {
        ...
        if (
            ...
            !anchor1 ||
            anchor0.z !== z ||
            anchor0.width !== map.width ||
            anchor0.height !== map.height ||
            anchor0.pitch !== pitch ||
            anchor0.distanceTo(this._containerAnchor0) >
                COLLISION_OFFSET_THRESHOLD ||
            anchor1.distanceTo(this._containerAnchor1) >
                COLLISION_OFFSET_THRESHOLD
        ) {
            return true;
        }
    ...
    _startCollision() {
        const map = this.getMap();
        this._coordCache = {};
        this._containerAnchor0 = new maptalks.Point(
            map.width / 3,
            map.height / 2,
        );
        this._containerAnchor1 = new maptalks.Point(
            (map.width * 2) / 3,
            map.height / 2,
        );
    ...
    _getMeshAnchor(meshKey) {
        ...
        if (meshContext && meshContext.anchor0) {
            ...
            const key0 = (anchor0[keyName] =
                anchor0[keyName] || anchor0.x + "," + anchor0.y);
            const key1 = (anchor1[keyName] =
                anchor1[keyName] || anchor1.x + "," + anchor1.y);
            let cp0 = this._coordCache[key0];
            let cp1 = this._coordCache[key1];
            if (!cp0 || !cp1) {
                const map = this.getMap();
                cp0 = this._coordCache[key0] =
                    map.coordToContainerPoint(anchor0);
                cp1 = this._coordCache[key1] =
                    map.coordToContainerPoint(anchor1);
            }
            cp0.z = anchor0.z;
            MESH_ANCHORS[0] = cp0;
            MESH_ANCHORS[1] = cp1;
    ...
    updateBoxCollisionFading(boxVisible, mesh, meshBoxes, mvpMatrix, boxIndex) {
        ...
        if (
            isEnableUniquePlacement &&
            this._isReplacedPlacement(meshKey, boxIndex)
        ) {
            return false;
        }
        ...
        let collision =
            collisionTags[meshKey] && collisionTags[meshKey][boxIndex];
        ...
        if (!isCollidedOnZooming && boxVisible) {
            const uncollidedOnZooming =
                zoomColliding && collision.collides === 0;
            if (this._canProceed || uncollidedOnZooming) {
                if (
                    this._meshCollisionStale ||
                    (collision && collision.z !== zoom)
                ) {
                    collision = null;
                }
                if (!collision) {
                    ...
                    let collides = 0;
                    for (let i = 0; i < l; i++) {
                        const { mesh, allElements, boxCount, start, end } =
                            meshBoxes[i];
                        const childCollision = this._isBoxVisible(
                            mesh,
                            allElements,
                            boxCount,
                            start,
        ...
        if (this.sceneConfig.fading) {
            ...
            if (!this._zoomingOut) {
                ...
                fadingOpacity = this._getBoxFading(
                    isForeground,
                    visible,
                    stamps,
                    boxIndex,
                ...
                if (visible) {
                    ...
                    if (
                        ...
                        if (isNil(fadingDuration)) {
                            fadingDuration =
                                DEFAULT_SCENE_CONFIG.fadingDuration;
                        }
                        if (isNil(fadeOutDelay)) {
                            fadeOutDelay = DEFAULT_SCENE_CONFIG.fadeOutDelay;
                        }
                        const timestamp = renderer.getFrameTimestamp();
                        const zoomEndFading = clamp(
                            1 -
                                (timestamp - fadeOutStart - fadeOutDelay) /
                                    fadingDuration,
                            0,
                            1,
                        );
        ...
        if (collision && layer.options["debugCollision"]) {
            this.addCollisionDebugBox(
                collision.boxes,
                collision.collides ? 0 : 1,
            );
        }
        ...
        if (visible || isFading) {
            ...
            if (
                !this._isIgnorePlacement(symbol, mesh, start) &&
                collision &&
                collision.boxes
            ) {
        ...
        if (visible) {
            ...
            for (let i = 0; i < l; i++) {
                const { mesh, allElements, start, boxStart, end, boxIndex } =
                    meshBoxes[i];
    ...
    _isBoxVisible(mesh, elements, boxCount, start, end, mvpMatrix, boxIndex) {
        const symbol = this.getSymbol(mesh.properties.symbolIndex);
        const isIgnorePlacement = this._isIgnorePlacement(
            symbol,
            mesh,
            elements[start],
        );
        const isAllowOverlap = this._isAllowOverlap(
            symbol,
            mesh,
            elements[start],
        );
        if (
            ...
            (isIgnorePlacement && isAllowOverlap)
        ) {
            return NO_COLLISION;
        }
        const collision = this.isBoxCollides(
            mesh,
            elements,
            boxCount,
            start,
    ...
    _getBoxFading(isForeground, visible, stamps, index) {
        ...
        if (isNil(fadeInDelay)) {
            fadeInDelay = DEFAULT_SCENE_CONFIG.fadeInDelay;
        }
        if (isNil(fadeOutDelay)) {
            fadeOutDelay = DEFAULT_SCENE_CONFIG.fadeOutDelay;
        }
    ...
    deleteMesh(meshes, keepGeometry) {
        ...
        if (Array.isArray(meshes)) {
            for (let i = 0; i < meshes.length; i++) {
                ...
                if (this._collisionContext)
                    delete this._collisionContext.tags[key];
    ...
    isCollides(box /*, tileInfo*/) {
        ...
        const bufferSize =
            this.sceneConfig.collisionBufferSize ||
            layer.options["collisionBufferSize"] ||
            0;
        if (bufferSize) {
    ...
    _addCollisionBox(box, visible) {
        ...
        const bufferSize =
            this.sceneConfig.collisionBufferSize ||
            this.layer.options["collisionBufferSize"] ||
            0;
        if (bufferSize) {
            box = bufferBox(BOX, box, bufferSize);
        }
    ...
    updateCollision(context) {
        ...
        if (
            map.isZooming() ||
            (this._zoomEndMeshes && this._zoomEndMeshes.length)
        ) {
    ...
    _prepareZoomEndMeshes() {
        ...
        if (!zooming && this._zooming) {
            ...
            this._zoomEndMeshes = this.scene
                .getMeshes()
                .filter(
                    (m) =>
                        !renderer.isForeground(m) &&
                        !m.properties.isLinePlacement,
                );
        ...
        if (zooming) {
            ...
            this._zoomingOut =
                this._preRes && map.getResolution() > this._preRes;
    ...
    _updateZoomMeshesLevel() {
        ...
        for (let i = 0; i < this._zoomEndMeshes.length; i++) {
            ...
            const level =
                tileInfo.z - tileZoom > 0
                    ? 2 * (tileInfo.z - tileZoom) - 1
                    : 2 * (tileZoom - tileInfo.z);
            mesh.properties.level = level;
            if (
                renderer.isForeground(mesh) ||
                (mesh._fadeOutStartTime &&
                    timestamp - mesh._fadeOutStartTime >
                        fadeOutDelay + fadingDuration)
            ) {
                delete mesh._fadeOutStartTime;
    ...
    isEnableCollision() {
        return (
            this.layer.options["collision"] && !!this.sceneConfig["collision"]
        );
    }
    ...
    _updateUniquePlacements() {
        ...
        const fn = (mesh, meshBoxes, matrix, boxIndex) => {
            ...
            if (placements[boxIndex] === undefined) {
                const key = this.getUniqueEntryKey(
                    mesh,
                    elements[start],
                    boxIndex,
                );
    ...
    _mergeUniquePlacements(meshes) {
        ...
        for (let i = 0; i < meshes.length; i++) {
            ...
            for (let j = 0; j < uniquePlacements.length; j++) {
                ...
                if (!placements) {
                    //用纯数组比对象性能更好
                    allPlacements[uKey] = [mesh, stamps, index];
                } else {
    ...
    _isReplacedPlacement(meshKey, index) {
        return (
            this._replacedPlacements &&
            this._replacedPlacements[meshKey] &&
            this._replacedPlacements[meshKey][index]
        );
    }
    ...
    _isHalo0(mesh) {
        ...
        const symbolDef = this.getSymbolDef(
            mesh.geometry.properties.symbolIndex,
        );
        const hasHaloRadius = isFnTypeSymbol(symbolDef.textHaloRadius);
        if (
            !mesh.geometry.properties.glyphAtlas ||
            !mesh.material.get("isHalo") ||
            (hasHaloRadius && mesh.geometry.properties.hasHalo)
        ) {
            return false;
        }
...
function getUniqueKey(key, scale) {
    return (
        Math.round(key[0] / scale / UNIQUE_TOLERANCE) *
            Math.round(key[1] / scale / UNIQUE_TOLERANCE) *
            (key[2] ? Math.round(key[2] / UNIQUE_TOLERANCE) : 1) +
        "-" +
        key[3]
    );
}
