...
vec4 packMipmapRefraction() {
    ...
    else if (level < 4.1) result.rgb = (vec4(decodeRGBM(texture2D(TextureRefractionBlur4, (min(gTexCoord, 1.0 - 1e+0 / uTextureRefractionBlur4Size.xy)) * uTextureRefractionBlur4Ratio), uRGBMRange), 1.0)).rgb;
    else if (level < 5.1) result.rgb = (vec4(decodeRGBM(texture2D(TextureRefractionBlur5, (min(gTexCoord, 1.0 - 1e+0 / uTextureRefractionBlur5Size.xy)) * uTextureRefractionBlur5Ratio), uRGBMRange), 1.0)).rgb;
    else if (level < 6.1) result.rgb = (vec4(decodeRGBM(texture2D(TextureRefractionBlur6, (min(gTexCoord, 1.0 - 1e+0 / uTextureRefractionBlur6Size.xy)) * uTextureRefractionBlur6Ratio), uRGBMRange), 1.0)).rgb;
    else if (level < 7.1) result.rgb = (vec4(decodeRGBM(texture2D(TextureRefractionBlur7, (min(gTexCoord, 1.0 - 1e+0 / uTextureRefractionBlur7Size.xy)) * uTextureRefractionBlur7Ratio), uRGBMRange), 1.0)).rgb;
    return result;
}
void main(void) {
