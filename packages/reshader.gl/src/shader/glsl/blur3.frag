...
vec4 gaussianBlur() {
    vec3 pixel = 0.24609375 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy), uRGBMRange), 1.0)).rgb;
    vec2 offset;
    vec2 blurDir = uPixelRatio.xy * uBlurDir.xy / uTextureOutputSize.xy;
    offset = blurDir * 1.3636363636363635;
    pixel += 0.322265625 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy + offset.xy), uRGBMRange), 1.0)).rgb;
    pixel += 0.322265625 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy - offset.xy), uRGBMRange), 1.0)).rgb;
    offset = blurDir * 3.1818181818181817;
    pixel += 0.0537109375 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy + offset.xy), uRGBMRange), 1.0)).rgb;
    pixel += 0.0537109375 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy - offset.xy), uRGBMRange), 1.0)).rgb;
