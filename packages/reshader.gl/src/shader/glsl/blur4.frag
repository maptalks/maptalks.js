...
vec4 gaussianBlur() {
    vec3 pixel = 0.2255859375 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy), uRGBMRange), 1.0)).rgb;
    vec2 offset;
    vec2 blurDir = uPixelRatio.xy * uBlurDir.xy / uTextureOutputSize.xy;
    offset = blurDir * 1.3846153846153846;
    pixel += 0.314208984375 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy + offset.xy), uRGBMRange), 1.0)).rgb;
    pixel += 0.314208984375 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy - offset.xy), uRGBMRange), 1.0)).rgb;
    offset = blurDir * 3.230769230769231;
    pixel += 0.06982421875 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy + offset.xy), uRGBMRange), 1.0)).rgb;
    pixel += 0.06982421875 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy - offset.xy), uRGBMRange), 1.0)).rgb;
    offset = blurDir * 5.076923076923077;
    pixel += 0.003173828125 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy + offset.xy), uRGBMRange), 1.0)).rgb;
    pixel += 0.003173828125 *  (vec4(decodeRGBM(texture2D(TextureBlurInput, gTexCoord.xy - offset.xy), uRGBMRange), 1.0)).rgb;
