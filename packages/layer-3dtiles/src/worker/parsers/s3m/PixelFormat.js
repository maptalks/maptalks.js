...
PixelFormat.createTypedArray = function (
    ...
    let constructor;
    let sizeInBytes = PixelDatatype.sizeInBytes(pixelDatatype);
    ...
    let size = PixelFormat.componentsLength(pixelFormat) * width * height;
...
PixelFormat.flipY = function (
    ...
    let flipped = PixelFormat.createTypedArray(
        pixelFormat,
        pixelDatatype,
        width,
        height
    );
    let numberOfComponents = PixelFormat.componentsLength(pixelFormat);
    let textureWidth = width * numberOfComponents;
    for (let i = 0; i < height; ++i) {
        let row = i * width * numberOfComponents;
        let flippedRow = (height - i - 1) * width * numberOfComponents;
